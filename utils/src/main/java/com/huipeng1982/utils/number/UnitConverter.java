package com.huipeng1982.utils.number;
...
public class UnitConverter {
    ...
    public static long toDurationMillis(String duration) {
        ...
        switch (unit) {
            ...
            case 'm':
                ...
                if (unitStr.length() >= UNIT_STR_LENGTH && unitStr.charAt(1) == UNIT_STR_CHAR_AT_1) {
                    return number;
                }
    ...
    public static String toSizeUnit(Long bytes, int scale) {
        ...
        if (bytes < M) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 4 : 5 + scale).append(".").append(scale).append("fk").toString(), bytes * 1d / K);
        }

        if (bytes < G) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 4 : 5 + scale).append(".").append(scale).append("fm").toString(), bytes * 1d / M);
        }

        if (bytes < T) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 4 : 5 + scale).append(".").append(scale).append("fg").toString(), bytes * 1d / G);
        }

        return String.format(new StringBuilder("%").append(scale == 0 ? 4 : 5 + scale).append(".").append(scale).append("ft").toString(), bytes * 1d / T);
    }
    ...
    public static String toTimeUnit(long millis, int scale) {
        ...
        if (millis < MILLIS_PER_MINUTE) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 2 : 3 + scale).append(".").append(scale).append("fs").toString(), millis * 1d / MILLIS_PER_SECOND);
        }

        if (millis < MILLIS_PER_HOUR) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 2 : 3 + scale).append(".").append(scale).append("fm").toString(), millis * 1d / MILLIS_PER_MINUTE);
        }

        if (millis < MILLIS_PER_DAY) {
            return String.format(new StringBuilder("%").append(scale == 0 ? 2 : 3 + scale).append(".").append(scale).append("fh").toString(), millis * 1d / MILLIS_PER_HOUR);
        }

        return String.format(new StringBuilder("%").append(scale == 0 ? 2 : 3 + scale).append(".").append(scale).append("fd").toString(), millis * 1d / MILLIS_PER_DAY);
