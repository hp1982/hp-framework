...
import com.huipeng1982.utils.base.SystemPropertiesUtil;
import com.huipeng1982.utils.base.type.Pair;
import com.huipeng1982.utils.collection.MapUtil;
...
public class NetUtil {
    ...
    private static final String NIF_LIST_SPLIT = ",";
    ...
    private static class LocalAddress {
        ...
        private static InetAddress findLocalAddressViaNetworkInterface() {
            ...
            try {
                Pair<InetAddress, Map<String, NetworkInterface>> addressAndInterfacemapPair = searchInetAddress(preferNamePrefix);
                if (addressAndInterfacemapPair.getLeft() != null) {
                    return addressAndInterfacemapPair.getLeft();
                }

                InetAddress inetAddress = findInetAddress(defaultNicList, addressAndInterfacemapPair.getRight());
                if (inetAddress != null) {
                    return inetAddress;
                }
        ...
        /**
         * 循环localhost.default.nic.list，找出合适的网卡
         */
        private static InetAddress findInetAddress(String defaultNicList, Map<String, NetworkInterface> candidateInterfaces) {
            for (String nifName : defaultNicList.split(NIF_LIST_SPLIT)) {
        ...
        /**
         * 循环NetworkInterfaces，找出合适的网卡
         */
        private static Pair<InetAddress, Map<String, NetworkInterface>> searchInetAddress(String preferNamePrefix) throws SocketException {

            InetAddress resultAddress = null;
            Map<String, NetworkInterface> candidateInterfaces = MapUtil.newHashMap();

            Pair<InetAddress, Map<String, NetworkInterface>> pair = null;
            ...
            for (Enumeration<NetworkInterface> allInterfaces = NetworkInterface.getNetworkInterfaces(); allInterfaces.hasMoreElements(); ) {
                ...
                if (name.startsWith(preferNamePrefix)) {
                    ...
                    if (resultAddress != null) {
                        pair = Pair.of(resultAddress, candidateInterfaces);
                        return pair;
                    }
                } else {
                    // 不是Prefer前缀，先放入可选列表
            ...
            pair = Pair.of(null, candidateInterfaces);
            return pair;
        }
